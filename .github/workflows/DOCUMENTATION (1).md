...
| 14 | **Entire pipeline silently operated on raw IPs instead of hostnames** | `dnsx -resp-only -a` strips the hostname and prints ONLY the resolved IP (confirmed against ProjectDiscovery's own documented example output) - `resolved.txt`/`all_subs.txt` and everything downstream (nuclei, JS analysis, API discovery, security analysis, URL collection) were built from IP addresses, not real subdomains, for multiple runs | Use `-resp` (not `-resp-only`) to keep `"hostname [ip]"` format, then `awk '{print $1}'` to extract the hostname column |
